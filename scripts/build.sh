#!/usr/bin/env bash
# Build the Wortkatze APK from the command line and stage it into dist/.
#
...
#               Python 3 (the vocabulary gate below).
set -euo pipefail
...
if [ "$VARIANT" = "release" ]; then
  ...
  OUT="dist/wortkatze-v${VERSION}-release.apk"
else
  TASK="assembleDebug"
  APK="app/build/outputs/apk/debug/app-debug.apk"
  OUT="dist/wortkatze-v${VERSION}-debug.apk"
fi
...
# The vocabulary is generated, and a stale vocab.json ships wrong words with no
# other symptom. Catch it here, not in review.
if command -v python3 >/dev/null 2>&1; then
  echo "==> checking the vocabulary is freshly generated"
  python3 tools/gen_vocab.py --check
else
  echo "-- python3 not found; skipping the vocabulary freshness check"
fi
