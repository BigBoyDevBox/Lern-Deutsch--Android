#!/usr/bin/env bash
#
# Build Wortkatze and install it on a connected Android device, then launch it.
#
# Handy when working from IntelliJ IDEA (which lacks Android Studio's Run
# button): connect the phone with USB debugging enabled (or pair over Wi-Fi
...
case "$VARIANT" in
  debug)   TASK="installDebug";   APP_ID="ch.lkmc.wortkatze.debug" ;;
  release) TASK="installRelease"; APP_ID="ch.lkmc.wortkatze" ;;
esac
